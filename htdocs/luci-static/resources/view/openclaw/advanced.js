...
return view.extend({
	render: function() {
		...
		const html =
			...
			_('<h2>⚙️ Terminal Configuration (Config Management)</h2>') +
			_('<p>Manage OpenClaw end-to-end through the interactive terminal (oc-config): AI model configuration, messaging channel setup (QQ, Telegram, Discord, ...), health checks and more.</p>') +
			'</div>' +
			'<div class="oc-console-info">' +
			_('<span class="label">Terminal service:</span>') +
			'<span class="value" id="oc-term-addr">-</span>' +
			'<span class="sep">|</span>' +
			_('<span class="label">Status:</span>') +
			_('<span id="oc-term-status-text">Checking...</span>') +
			_('<a id="oc-term-open-btn" class="btn-open" href="#" target="_blank" rel="noopener">↗ Open terminal in new window</a>') +
			'</div>' +
			...
			'</div>' +
			_('<div style="font-size:13px;color:#555;line-height:1.5;margin-bottom:6px;">A browser connecting to the Control UI triggered the secure pairing policy — approve it right here with one click instead of searching for the Request ID in the terminal below.</div>') +
			...
			_('<button type="button" id="oc-adv-pairing-approve-all-btn" style="padding:6px 14px;background:#52c41a;color:#fff;border:none;border-radius:4px;font-weight:600;font-size:12px;cursor:pointer;">⚡ Approve all pairings</button>') +
			_('<button type="button" id="oc-adv-pairing-refresh-btn" style="padding:6px 10px;background:#f0f2f5;color:#333;border:1px solid #d0d7de;border-radius:4px;font-size:12px;cursor:pointer;">🔄 Refresh</button>') +
			'</div></div>' +
			...
			_('<div class="oc-terminal-loading" id="oc-terminal-loading">⏳ Connecting to the config terminal...</div>') +
			'</div></div>';
		...
		Promise.all([api.getToken(), api.status()]).then(function(results) {
			...
			if (d.pty_running) {
				if (self.statusTextEl)
					self.statusTextEl.innerHTML = _('<span style="color:#1a7f37;">● Running</span>');
				self.showIframe();
			}
			else {
				if (self.statusTextEl)
					self.statusTextEl.innerHTML = _('<span style="color:#cf222e;">● Not running</span>');
				self.loading.innerHTML = _('<div style="text-align:center;padding:24px;color:#cf222e;">❌ The config terminal is not running<br/>') +
					_('<span style="font-size:12px;color:#999;">Enable and start the service under "Basic Settings" first.</span></div>');
			}
		}).catch(function(e) {
			if (self.statusTextEl)
	...
	showIframe: function() {
		...
		if (isHttps) {
			this.loading.innerHTML = '<div style="text-align:center;padding:40px 20px;color:#abb2bf;">' +
				...
				_('<div style="font-size:15px;margin-bottom:8px;color:#e5c07b;font-weight:600;">Due to browser security policy, the HTTP terminal cannot be embedded directly on an HTTPS page</div>') +
				...
				_('<div style="margin-top:16px;font-size:12px;color:#5c6370;">Tip: you can also log into the LuCI web UI over HTTP (http://') + window.location.hostname + _(') and use the embedded terminal right here.</div>') +
				'</div>';
			return;
		}
